...
def paginacao() -> None:
    ...
    driver.implicitly_wait(7)
    for i in range(1,3):
        url = f'https://www.leroymerlin.com.br/search?term=Gart&searchTerm=Gart&searchType=default&page={i}'
        ...
        try:
            ...
            print("error")
    data = pd.DataFrame({"URLS":lista_urls})
    data.to_excel("tarketeroy.xlsx")


def produtos_leroy() -> None:
    lista_dicts = []
    driver.implicitly_wait(7)
    data = pd.read_excel(r'D:\produtoshausz\elianeleroy.xlsx')
    lista_urls = data.to_dict('records')
    for listas in lista_urls:

        try:
            driver.get(listas['URLS'])
           
            dict_produtos = {}
            time.sleep(1)

            dict_produtos['PAGINAPRODUTO'] = listas['URLS']
            try:
                preco = driver.find_elements(By.XPATH,'/html/body/div[9]/div/div[1]/div[3]/div[1]/div/div[1]/div[1]/div/span')
